...
| **2** | **PC 저장소 청소** — 드라이브·캐시·임시 폴더 분석, 삭제 전 한 줄 미리보기, 선택 항목만 정리 (아래 절 참고) |
| **3** | `boot.config` 만 패치 (백업 세션 + `summary.json`) |
| **4** | 예전 백업 세션 선택 후 레지·전원·`boot.config` 재적용 |
| **5** | 세션에 있는 `boot.config` 백업만 골라 게임 쪽으로 복원 |
| **6** | 설치 경로만 다시 스캔 |
| **7** | 실행 파일 수동 지정 (경로 입력 또는 파일 선택) |
| **Q** | 종료 |

## PC 저장소 청소 (`[2]`)

이터널 리턴 설정과 **별도**로, 로컬 디스크·일반적인 임시/캐시 위치를 **스캔**하고, **삭제 전에 항목별 한 줄 요약**을 본 뒤, 사용자가 고른 항목만 **부분 삭제**합니다.  
**기본 패키지([1])의 `TempCleanupService`는 변경하지 않습니다.** (정리 로직은 `EROptimizer.Core.StorageCleanup`)

삭제 실패(파일 사용 중 등)는 로그에 **사유별로 건수만** 묶어 `삭제 실패 : … [n건] - SKIP.` 형식으로 남깁니다.

### 이 메뉴가 하는 일 (MVP)

- 드라이브별 용량·C: 여유 부족 안내(5GB 미만을 ‘부족’으로 표시)
- `%TEMP%`, `C:\Windows\Temp`, `C:\Windows.old` 존재/용량, C: 휴지통(가능할 때만 추정)
- `C:\NVIDIA`, `C:\AMD` 루트만 — **설치 압축 해제 캐시 후보**(실제 설치된 드라이버 본체와 구분)
- Delivery Optimization 캐시(일반적으로 알려진 ProgramData 경로만 스캔·선택 시만 해당 경로 하위 삭제)
- DirectX Shader Cache(`…\AppData\Local\D3DSCache`)
- Windows 오류 보고 대기/보관 + 사용자 로컬 크래시 덤프(일반적인 경로만)
- 실행 전 `logs\cleanup_preview_<세션>.json`, 실행 후 `logs\cleanup_result_<세션>.json`, 상세는 `logs\optimizer_<세션>.log`

삭제는 항목별로 진행하며 사용 중·권한 없음은 건너뜁니다. 한 파일이 실패해도 나머지는 계속 진행합니다.

### 하지 않는 일 (MVP)

| 구분 | 내용 |
|------|------|
| 직접 삭제 안 함 | `Windows.old` 실제 삭제, `WinSxS` 직접 삭제, DriverStore 접근, DISM 정리·**`/ResetBase`** |
| 건드리지 않음 | `Program Files`·`(x86)` 내 NVIDIA/AMD 앱 폴더, 브라우저 데이터, 문서·바탕화면·다운로드 등 사용자 폴더 일반 위치, Steam·이터널 리턴 설치 트리 |
| 없음 | 레지스트리 클리너, RAM 클리너, 서비스 대량 비활성화, 방화벽·Defender·UAC 조작 |

### 안전장치 (요약)

- 삭제 대상 경로는 **FullPath 정규화** 후 검사합니다.
- **junction/symlink(reparse point)** 는 따라가지 않으며 해당 항목은 건너뜁니다.
- 스팀 `steamapps` 경로와 이터널 리턴 설치 폴더(탐색 결과 기준) 아래는 삭제하지 않습니다.
- 허용된 루트 밖으로 나가는 경로는 삭제하지 않습니다.
- 삭제형 정리는 **완전 복구를 보장하지 않습니다.** JSON·로그는 참고용 기록입니다.

### 알려진 한계·SmartScreen

- 표시 용량과 실제 확보 용량은 파일 잠금 등으로 달라질 수 있습니다.
- 서명되지 않은 실행 파일은 Defender·SmartScreen 경고가 날 수 있습니다. 출처와 해시를 직접 확인하세요.
- 릴리스 검증용 SHA256은 `Get-FileHash -Algorithm SHA256 .\EROptimizer.exe` 로 계산해 배포 문서에 적는 방식을 권장합니다.
...
자동 실패 시 **`[7]`** 에서 경로를 붙여 넣거나 파일 대화상자로 지정합니다. **`[6]`** 은 설정을 바꾸지 않고 탐색만 다시 합니다.
...
- **`[4]`** — `registry_backup.json` 으로 레지 되돌림 + `power_plan_backup.txt` 의 GUID로 `powercfg /setactive`.
- **`boot.config`** — 해당 세션 `files\boot.config.bak_*` 중 경로 문자열 기준 맨 앞 항목 사용. **`[5]`** 는 목록에서 번호로 고름.
...
게임 패치로 `boot.config` 가 바뀌면 병합 결과도 달라질 수 있습니다. 그럴 때는 다시 실행하거나 메뉴 `[3]` 으로 boot.config 만 적용하면 됩니다.
